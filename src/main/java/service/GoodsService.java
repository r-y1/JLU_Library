...
//    改:借阅时减少库存
    public void delStock(String name,int amount){//货物的name
        try{
            gDao.delStock(name,amount);
        } catch (SQLException e){
            e.printStackTrace();
        }
    }
    //归还时增加库存
    public void addStock(String name,int amount){//货物的name
        try{
            gDao.addStock(name,amount);
        } catch (SQLException e){
            e.printStackTrace();
        }
    }
